...
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2,3'   #'0,1,2' #'2,1,0' 
...
def train(args, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs): 
        for step,batch in enumerate(train_dataloader):
            ...
            code_vec,gcode_vec = model(code_inputs=code_inputs)
            nl_vec,gnl_vec = model(nl_inputs=nl_inputs)
            
            
            #calculate scores and loss
            #scores = torch.einsum("ab,cb->ac",gcode_vec,gnl_vec)

            #loss_fct = CrossEntropyLoss()

            loss_function = NormSoftmaxLoss(temperature=0.01)


            loss5 = compute_kl_loss(nl_vec,gnl_vec)
            loss6 = compute_kl_loss(code_vec,gcode_vec)

            loss3 = loss_function(sim_matrix(nl_vec,code_vec))
            loss4 = loss_function((sim_matrix(nl_vec,code_vec)).T)
    
            loss2 = loss_function(sim_matrix(gnl_vec,gcode_vec))
            
            loss1 = loss_function((sim_matrix(gnl_vec,gcode_vec)).T)
            
            loss = loss1 + loss2 + loss3 + loss4 + loss5 + loss6
            #loss = loss3 + loss4 + loss5 + loss6
            #loss = loss1 + loss2
...
def evaluate(args, model, tokenizer,file_name,eval_when_training=False):
    ...
    for batch in query_dataloader:  
        nl_inputs = batch[1].to(args.device)
        with torch.no_grad():
            _, gnl_vec = model(nl_inputs=nl_inputs) 

            gnl_vecs.append(gnl_vec.cpu()) 

    for batch in code_dataloader:
        code_inputs = batch[0].to(args.device)    
        with torch.no_grad():
            _,gcode_vec = model(code_inputs=code_inputs)

            gcode_vecs.append(gcode_vec.cpu()) 

    
    gcode_vecs = np.concatenate(gcode_vecs,0)
    gnl_vecs = np.concatenate(gnl_vecs,0)
    scores = np.matmul(gnl_vecs,gcode_vecs.T)
